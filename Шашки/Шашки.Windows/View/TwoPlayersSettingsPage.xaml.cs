...
namespace Шашки.View
{
    ...
    /// </summary>
    public sealed partial class TwoPlayersSettingsPage : Page,INotifyPropertyChanged
    {
        ...
        private TwoPlayersSettings settings;
        public TwoPlayersSettingsPage()
        {
        ...
        async private void uploadImage_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (file != null)
            {
                using (IRandomAccessStream stream = await file.OpenAsync(Windows.Storage.FileAccessMode.Read))
                {
                    ...
                    settings.ImageOfFirstPlayer = bitmapImage;
        ...
        async private void uploadImageOfSecond_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (file != null)
            {
                ...
                {
                    BitmapImage bitmapImage = new BitmapImage();
                    settings.ImageOfSecondPlayer = bitmapImage;
                    bitmapImage.DecodePixelWidth = 600;
                    await bitmapImage.SetSourceAsync(stream);
        ...
        private void time_ValueChanged(object sender, RangeBaseValueChangedEventArgs e)
        {
            ...
            settings.TimeOfMove = TimeSpan.FromMinutes(0);
            settings.TimeOfGame = TimeSpan.FromMinutes(time.Value);
        }
        ...
        private void timeOfOneMove_ValueChanged(object sender, RangeBaseValueChangedEventArgs e)
        {
            if (time.Value != 0)
                time.Value = 0;
            settings.TimeOfGame = TimeSpan.FromMinutes(0);
            settings.TimeOfMove = TimeSpan.FromMinutes(timeOfOneMove.Value);
        }
